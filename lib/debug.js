...
module.exports = function(next, use) {
  ...
  // adds useful debug output to any mdoq req
  ...
  write(this.url || req.url);

  if(req.query) {
    write('query', 1);
    write(color('light', util.inspect(req.query)), 2);
  }

  if(req.data) {
    write('data', 1);
    write(color('light', util.inspect(req.data)), 2);
  }

  // wrap callback for error reporting
  
  var cb = this.req.callback;
  
  this.req.callback = function(err, res) {
